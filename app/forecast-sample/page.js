...
export default function ForecastPage() {
  ...
  const cagrList = useMemo(() => {
    ...
    if (hasLinear) {
      ...
      if (linCagr !== null)
        results.push({ name: "Forecast (Stats)", color: "#F58C1F", cagr: linCagr });
    }
    if (hasScore) {
      const scoreCagr = computeCagr("forecastScore");
      if (scoreCagr !== null)
        results.push({ name: "Forecast (Survey)", color: "#23DD1D", cagr: scoreCagr });
    }
    if (hasAi) {
    ...
    if (hasRaceInsights) {
      ...
      if (riCagr !== null)
        results.push({ name: "Forecast (Race Insights)", color: "#8A2BE2", cagr: riCagr });
    }
  ...
  const legendPayload = useMemo(() => {
    ...
    if (hasScore)
      items.push({ value: "Forecast (Survey-based)", type: "line", color: "#23DD1D" });
    if (hasAi)
      items.push({ value: "Forecast (AI)", type: "line", color: "#0080FF" });
    if (hasRaceInsights)
      items.push({ value: "Forecast (Race Insights)", type: "line", color: "#8A2BE2" });
    return items;
        ...
        <div className="container mt-1">
          ...
          <div className="app-header d-flex justify-content-between align-items-center">
            ...
            <div className="nav-buttons">
              <button className="nav-btn" onClick={() => router.push("/score-card")}>
                <FaClipboardList className="btn-icon" /> Build Your Own Tailored Forecast
              </button>
              <button className="nav-btn" onClick={() => router.push("/reports")}>
          ...
          <div className="selectors d-flex align-items-center gap-3">
            ...
            >
              <span style={{ color: "white" }}>
                {selectedDatasetId ? datasetMap[selectedDatasetId].label : "Categories"}
              </span>
            ...
            >
              ...
              <div className={`chart-dropdown ${isRegionsHovering ? "open" : ""}`}>
                ...
                >
                  {selectedRegions.length === allRegions.length ? "Remove all" : "Select all"}
                </div>
                  ...
                  <div key={grp} style={{ marginBottom: 8, color: "white" }}>
                    ...
                    >
                      <input
                        type="checkbox"
                        className="me-2"
                        checked={nodes.every((n) => selectedRegions.includes(n.name))}
                        onChange={() => {
                          // toggle entire group
                          const allInGroup = nodes.every((n) => selectedRegions.includes(n.name));
                          if (allInGroup) {
                            // remove these from selectedRegions
                            setSelectedRegions((prev) =>
                              prev.filter((reg) => !nodes.map((n) => n.name).includes(reg))
                            );
                          } else {
                            // add missing ones
                            setSelectedRegions((prev) => [
                              ...new Set([...prev, ...nodes.map((n) => n.name)]),
                            ]);
                          }
                        }}
                      />
          ...
          {/* CHART */}
          <div className="mt-3">
            {selectedDatasetId && selectedRegions.length > 0 && selectedGraphId && (
                        ...
                        >
                          <defs>
                            <linearGradient id="histGrad" x1="0" y1="0" x2="0" y2="1">
                              <stop
                                offset="0%"
                                stopColor={hasLinear && hasScore ? "#D64444" : "#1039EE"}
                                stopOpacity={0.9}
                              />
                              <stop
                                offset="100%"
                          ...
                          <YAxis
                            ...
                            tick={{
                              fill: isMobile ? "rgba(255,255,255,0.5)" : "#FFC107",
                              fontSize: isMobile ? 10 : 12,
                              dx: isMobile ? 30 : 0,
                            }}
                            domain={["auto", "auto"]}
